...
//  LetsOrder
//
//  Created by xtan on 3/18/11.
//  Copyright 2011 科脉远望信息技术(北京)有限公司. All rights reserved.
//
...
+(BOOL)identifyChinaMobilePhoneNumber:(NSString *)str{
    ...
    NSString * regex = @"^((13[0-9])|(17[0-9])|(14[0-9])|(10[0-9])|(15[^4,\\D])|(18[0-9]))\\d{8}$";
//    @"^[0-9]([,-]|[0-9])+$";
...
//    @"^[A-Za-z0-9_.-]{1,}@{1}[A-Za-z0-9_.-]{1,}.{1}[A-Za-z.]{1,}$";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
    return [pred evaluateWithObject:str];
}
...
+ (UIImage *)filterQRImageWithInputText:(NSString *)text withLogo:(UIImage *)logoImage{
    
    
   UIImage *qrimage = [self filterQRImageWithInputText:text];
    
    UIGraphicsBeginImageContext(qrimage.size);
    // 将二维码图片画上去
    [qrimage drawInRect:CGRectMake(0, 0, qrimage.size.width, qrimage.size.height)];
    
    [logoImage drawInRect:CGRectMake((qrimage.size.width - 80) / 2, (qrimage.size.width - 80) / 2, 80, 80)];
    // 获取最终的图片
...
+ (NSString *)getFormatDateStringFromString:(NSString *)date{
    
    if ([[date substringToIndex:10] isEqualToString:[[Utilities nowString] substringToIndex:10]]) {
        
        return NSLocalizedString(TODAY, @"");
        
    }
//    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    NSString *str = [date substringWithRange:NSMakeRange(5, 2)];
    int month = [str intValue];
    NSString *monthStr = @"";
    switch (month) {
        case 1:
            monthStr = NSLocalizedString(JANUARY, @"");
            break;
        case 2:
            monthStr = NSLocalizedString(FEBRUARY, @"");
            break;
        case 3:
            monthStr = NSLocalizedString(MARCH, @"");
            break;
        case 4:
            monthStr = NSLocalizedString(APRIL, @"");
            break;
        case 5:
            monthStr = NSLocalizedString(MAY, @"");
            break;
        case 6:
            monthStr = NSLocalizedString(JUNE, @"");
            break;
        case 7:
            monthStr = NSLocalizedString(JULY, @"");
            break;
        case 8:
            monthStr = NSLocalizedString(AUGUST, @"");
            break;
        case 9:
            monthStr = NSLocalizedString(SEPTEMBRT, @"");
            break;
        case 10:
            monthStr = NSLocalizedString(OCTOBER, @"");
            break;
        case 11:
            monthStr = NSLocalizedString(NOVEMBER, @"");
            break;
        case 12:
            monthStr = NSLocalizedString(DECEMBER, @"");
            break;
            
        default:
            break;
    }
    return [NSString stringWithFormat:@"%@\n%@",[date substringWithRange:NSMakeRange(8, 2)],monthStr];
}
...
+ (NSString *)JPGtimedVersionUniqueNameWithType:(NSString *)type{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *result=[NSString stringWithFormat:@"%@/%@%@%@", docPath, MEDIA_BASE, IMAGE_DIR, photoName];

    if (![fileManager isExecutableFileAtPath:dir]) {
        BOOL success=[fileManager createDirectoryAtPath:dir withIntermediateDirectories:YES attributes:nil error:nil];
        if (!success) {
            result=@"";
        }
    }
...
+ (UIAlertController *)actionSheetWithLocationFind:(CLLocationCoordinate2D)location withAddress:(NSString *)address{
    ...
    if ( [[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"http://maps.apple.com/"]])
    {
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"苹果地图" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            ...
            MKMapItem *toLocation = [[MKMapItem alloc] initWithPlacemark:[[MKPlacemark alloc] initWithCoordinate:coordinate addressDictionary:[NSDictionary dictionaryWithObjectsAndKeys: address,@"postalAddresses", nil]]];
    ...
    if ( [[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"iosamap://"]])
    {
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"高德地图" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            ...
            NSString *urlString = [[NSString stringWithFormat:@"iosamap://navi?sourceApplication=%@&backScheme=%@&lat=%f&lon=%f&dev=0&style=2&name=%@",appName,urlScheme,coordinate.latitude, coordinate.longitude, address] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
+ (UIAlertController *)actionSheetWithLocation:(CLLocationCoordinate2D)location withAddress:(NSString *)address{
    ...
    if ( [[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"baidumap://"]])
    {
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"百度地图" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            ...
            CLLocation *marsLocation = [[CLLocation alloc]initWithLatitude:coordinate.latitude longitude:coordinate.longitude];
            CLLocationCoordinate2D BDCoodinate = [marsLocation wgs84ToBd09:marsLocation.coordinate];

             NSString *urlString = [[NSString stringWithFormat:@"baidumap://map/marker?location=%f,%f&title=位置&content=%@&src=途遇|%@",BDCoodinate.latitude, BDCoodinate.longitude, address, appName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    if ( [[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"iosamap://"]])
    {
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"高德地图" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            CLLocation *marsLocation = [[CLLocation alloc]initWithLatitude:coordinate.latitude longitude:coordinate.longitude];
            CLLocationCoordinate2D marsCoordinat=[marsLocation MarsGS2WorldGS:marsLocation.coordinate];
            NSString *urlString = [[NSString stringWithFormat:@"iosamap://viewMap?sourceApplication=%@&backScheme=%@&poiname=%@&lat=&lat=%f&lon=%f&dev=1&name=%@",appName,urlScheme,address,marsCoordinat.latitude, marsCoordinat.longitude, address] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
            
//            NSString *urlString = [[NSString stringWithFormat:@"http://uri.amap.com/marker?position=%f,%f&name=%@&src=%@&coordinate=%@&callnative=1",coordinate.longitude,coordinate.latitude,address,appName, @"gaode"] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
